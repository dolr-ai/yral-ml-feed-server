...
def run(port=50059):
    ...
    with grpc.insecure_channel(f"localhost:{port}") as channel:
        ...
        request = video_recommendation_pb2.MLFeedRequestV4(
            user_principal_id="test_user_jay",
            watch_history=[
            ...
            success_history=[
                video_recommendation_pb2.SuccessHistoryItemV4(
                    video_id=i, item_type="like_video", percent_watched=random.random()*100
                )
            ...
            filter_posts=[
                ...
                for post_id, publisher_user_id, video_id in filter_responses
            ],
            num_results=25,
        )
        try:
            response = stub.get_ml_feed_clean_v4(request)
            response_nsfw = stub.get_ml_feed_nsfw_v4(request)
            response_combined = stub.get_ml_feed_combined_v4(request)
