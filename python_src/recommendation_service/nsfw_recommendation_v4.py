...
where video_id in ({video_ids_string})"""
        ...
        # Join the results with nsfw_probability
        if not mdf.empty and not rdf.empty:
            mdf = pd.merge(mdf, rdf[['video_id', 'nsfw_probability']], on='video_id', how='left')
        
        return mdf["post_id publisher_user_id video_id nsfw_probability".split()].to_dict("records")
...
--        LIMIT {search_breadth}
        ...
        try:
            ...
            _LOGGER.warning(f"Error in nsfw vector search query: {e}")
            return []
...
--        LIMIT {search_breadth}
    ...
    def get_collated_recommendation(
        ...
        Returns:
            MLFeedResponseV4: A response containing the recommended feed items.
        """
        sample_uris = (
        ...
        if self.logging:
            ...
            _LOGGER.info("[" + log_timestamp + "] [ID: " + log_id + "] NSFW Feed lengths - "
            ...
            print("NSFW Similar videos: " + " ".join(similar_videos))

        def create_feed_response(feed_items):
            return video_recommendation_pb2.MLFeedResponseV4(
                feed=[
                    video_recommendation_pb2.MLPostItemResponseV4(
        ...
        # for debugging
        if self.logging:
            _LOGGER.info(f"Length of returned NSFW feed: {len(deduped_sampled_feed)}")
        ...
        if self.logging:
            ...
            log_file = os.path.join(self.log_dir, f"nsfw_recommendation_v4_{timestamp}_{user_principal_id}_{uuid.uuid4()}.json")
            ...
            _LOGGER.info(f"NSFW recommendation V4 data logged to {log_file}")
...
if __name__ == "__main__":
    ...
    ] * 1  # outer prefix for variables that are also used in the logic code
    ...
    watch_history = [
        video_recommendation_pb2.WatchHistoryItemV4(video_id=i)
        for i in outer_videos_watched
    ]

    success_history = [
        video_recommendation_pb2.SuccessHistoryItemV4(
    ...
    filter_posts = [
        video_recommendation_pb2.MLPostItemV4(
    ...
    # input requests
    request = video_recommendation_pb2.MLFeedRequestV4(
    ...
    recommender = NsfwRecommendationV4()
    ...
    _LOGGER.info(
        f"Time required to get the NSFW recommendation: {end_time - start_time:.2f} seconds"
    )
