...
pub async fn get_coldstart_nsfw_cache_noinput_impl(
    ...
    let feed_v3 = ml_feed_cache
        .get_cache_items_v3_resilient(GLOBAL_CACHE_NSFW_KEY_V2, post_index, post_index + 1)
        .await
...
pub async fn get_coldstart_nsfw_cache_input_impl(
    ...
    let global_cache_nsfw_feed_v3 = ml_feed_cache
        .get_cache_items_v3_resilient(GLOBAL_CACHE_NSFW_KEY_V2, 0, MAX_GLOBAL_CACHE_LEN)
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get global cache nsfw feed: {}", e))?;

    // Convert PostItemV3 to PostItemV2, filtering out non-u64 post_ids
    let global_cache_nsfw_feed = convert_post_items_v3_to_v2(global_cache_nsfw_feed_v3);
